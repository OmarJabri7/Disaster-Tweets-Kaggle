...
from models.models import lstm_v1, lstm_v2, lstm_v3, lstm_v4, lstm_v5, lstm_v6, cnn_v1, cnn_v2, cnn_v3, cnn_v0, \
    best_cnn, bnn_v1, hybrid_v1, bert_v1, bert_v2, bert_v3, bert_v4, bert_v5
...
def create_w2vc(sentences, vector_size=200):
    ...
    word_model = Word2Vec(
        sentences, vector_size=vector_size, min_count=1, window=5)
...
def vectorize_data(x, vectorizer):
    if vectorizer is CountVectorizer:
        vectorizer = CountVectorizer(
            decode_error='ignore', lowercase=True, min_df=2)
...
def model_data(model_cls, params, grid_cv=False, x=None, y=None):
    ...
    if grid_cv and x is not None and y is not None:
        grid_search = GridSearchCV(
            model, param_grid=params, n_jobs=-1, verbose=1)
        grid_search.fit(x, y)
...
def train_net(preprocessor, reshape=False, split=False, model_name="normal", is_glove=False, is_create_w2vc=False):
    ...
    if split:
        X_train, X_val, y_train, y_val = train_test_split(
            X_train.text, y_train, test_size=0.1, random_state=42)

        print(
            f"Numbers of NO disasters {(y_train==0).sum()/len(y_train)*100}%")
    ...
    if(model_name != "bert"):
        ...
        train_tf_dataset = tf.data.Dataset.from_tensor_slices(
            (dict(X), y_tensors))
        X_val = tokenizer(X_val.to_list(), truncation=True, max_length=max(
            len_train_words), padding="max_length", return_tensors='tf')
        y_val_tensors = tf.convert_to_tensor(y_val[:])
        val_tf_dataset = tf.data.Dataset.from_tensor_slices(
            (dict(X_val), y_val_tensors))
    callback = tf.keras.callbacks.EarlyStopping(
        monitor='val_loss',)  # min_delta= 1e-1)
    ...
    model = bert_v5(max(len_train_words))
    if model_name != "bert":
        ...
        # validation_split=0.2, )
    if split:
        score, acc = model.evaluate(val_tf_dataset,
